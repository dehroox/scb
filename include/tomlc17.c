...
static void scan_init(scanner_t* sp,
                      const char* src,
                      int len,
                      char* errbuf,
                      int errbufsz);
...
static toml_datum_t* tab_emplace(toml_datum_t* tab,
                                 span_t key,
                                 const char** reason) {
...
static int tab_add(toml_datum_t* tab,
                   span_t newkey,
                   toml_datum_t newvalue,
                   const char** reason) {
    assert(tab->type == TOML_TABLE);
...
static int datum_copy(toml_datum_t* dst,
                      toml_datum_t src,
                      pool_t* pool,
                      const char** reason) {
...
static int datum_merge(toml_datum_t* dst,
                       toml_datum_t src,
                       pool_t* pool,
                       const char** reason) {
...
static toml_datum_t* descend_keypart(parser_t* pp,
                                     int lineno,
                                     toml_datum_t* toptab,
                                     keypart_t* keypart,
                                     bool stdtabexpr) {
...
static int parse_inline_array(parser_t* pp,
                              token_t tok,
...
static int parse_inline_table(parser_t* pp,
                              token_t tok,
                              toml_datum_t* ret_datum) {
...
static int read_time(const char* p,
                     int* hour,
                     int* minute,
                     int* second,
                     int* usec) {
